...
parser.add_argument('--path',
                    help='Path to serialized json CDDT data structure')
...
class SliceScroller(object):
    ...
    def onscroll(self, evt):
        print("Slice: %s  Theta: %s" %
              (self.ind, self.cddt.slices[self.ind].theta))
    ...
    def update(self):
        ...
        if not isinstance(self.ddts[self.ind], np.ndarray):
            ...
            self.ddts[self.ind] = np.sqrt(
                self.cddt.slices[self.ind].make_ddt(True)).transpose()

        ys = map(len, self.cddt.slices[self.ind].zeros)
        compression_factor = 2 * self.cddt.map.width * self.cddt.map.height / (
            sum(ys))
        ...
        self.ax1.imshow(self.ddts[self.ind],
                        cmap="gray",
...
if __name__ == '__main__':
    ...
    ddt_img_color = np.zeros((ddt_img.shape[0], ddt_img.shape[1], 3),
                             dtype=np.uint8)
    ...
    lut_img_color = np.zeros((lut_img.shape[0], lut_img.shape[1], 3),
                             dtype=np.uint8)
...
if __name__ == '__main__':
    ...
    w = 1350
    img = np.power(
        cddt.slices[3].make_ddt(reversed_dir=True).transpose()[120:120 +
                                                               w, :w], 0.7)
