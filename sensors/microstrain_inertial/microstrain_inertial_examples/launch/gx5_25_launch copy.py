...
def generate_launch_description():
    ...
    # Evaluate the launch argument into a string
    config_file_launch_config = LaunchConfiguration('config_file')

    def get_config_file_path():
        # Create a dummy context to evaluate the LaunchConfiguration
        context = launch.LaunchContext()
        return config_file_launch_config.perform(context)

    # Convert the evaluated LaunchConfiguration to a string
    config_file_path = get_config_file_path()

    launch_description.append(
        DeclareLaunchArgument(
            'namespace',
            default_value=gx5_namespace,
            description='Namespace to use when launching the nodes in this launch file'))
    launch_description.append(
        DeclareLaunchArgument(
            'node_name',
            default_value=nodename,
    ...
    microstrain_node = Node(
        ...
        parameters=[
            # Load the default params file manually, since this is a ROS params file,
            # we will need to load the file manually
            yaml.safe_load(open(_DEFAULT_PARAMS_FILE, 'r')),
            yaml.safe_load(open(config_file_path, 'r')),
